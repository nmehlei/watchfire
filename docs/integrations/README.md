...
| [SSH (watchfire user)](ssh.md) | `WATCHFIRE_SSH_PRIVATE_KEY_B64` | [`ssh-keygen-watchfire.sh`](../../scripts/setup/ssh-keygen-watchfire.sh) | ❌ missing |
...
```bash
./scripts/setup/azure-sp.sh watchfire-reader-main <sub-id-1> <sub-id-2> >> .env
```
...
- **SSH key env shape**: `WATCHFIRE_SSH_PRIVATE_KEY_B64` (base64-encoded ed25519 private key). Single key reused across all Hetzner accounts (one key, many forced-command authorized_keys entries).
