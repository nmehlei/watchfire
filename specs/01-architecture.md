...
│                                                                 │
│   Nginx Proxy Manager ──► ┌──────────────────────────────┐      │
│   (TLS, routes            │        watchfire (container)      │      │
│    watchfire.example.com       │  ┌────────────────────────┐  │      │
│    → watchfire:8080)           │  │  HTTP server (Watch)   │  │      │
│                           │  │  node-cron (Nightly)   │  │      │
│                           │  │  startup catch-up      │  │      │
│                           │  └──────────┬─────────────┘  │      │
│                           │             │                │      │
...
2. **Cron scheduler** (`node-cron`) that triggers `runNightly()` on `WATCHFIRE_NIGHTLY_CRON` (default `30 2 * * *`, daily) Europe/Berlin.
3. **Startup catch-up** that checks the `runs` table — if the last successful nightly is older than `WATCHFIRE_CATCHUP_STALE_HOURS` (default 24h), it fires one immediately (covers deploys, crashes, OOM kills during the cron window). A deployment running a sparser cron must raise this to match, or a restart between scheduled runs triggers an unplanned extra sweep.
...
10. Archive transcript to `/var/lib/watchfire/transcripts/`.
...
2. Query last successful nightly. If older than `WATCHFIRE_CATCHUP_STALE_HOURS` (default 24h) → trigger a catch-up run before entering normal loop.
...
TLS termination and routing are handled by NPM, not a Cloudflare Tunnel. One proxy host rule in the NPM UI maps `watchfire.example.com` → `watchfire:8080`. The Watchfire container joins the existing `nginx-proxy-manager_default` Docker network so NPM can reach it by service name.
...
services:
  watchfire:
    image: ghcr.io/your-org/watchfire:{{ watchfire_image_tag }}
    restart: unless-stopped
    volumes:
      - watchfire-data:/var/lib/watchfire                                  # SQLite + transcripts
      - /etc/watchfire/secrets/watchfire.env:/etc/watchfire/secrets/watchfire.env:ro
    env_file: /etc/watchfire/secrets/watchfire.env
    networks:
      - default
...
**DNS**: one CNAME record `watchfire.example.com` → `ops-host.example.com`, managed by Terraform via the shared `modules/azure/cname_record` module in your-iac-repo. The DNS zone itself already exists in `shared-app-plan-RG`.

**Migration to another host**: export the `watchfire-data` volume, copy `/etc/watchfire/`, point the Ansible inventory at the new host, repoint the NPM proxy rule. One evening of work.
...
| **your-iac-repo** | Azure DevOps (`your-org/your-iac-repo`)    | `solutions/watchfire/{terraform,ansible,deploy.sh}` — infra + ansible-vault secrets, local `deploy.sh` |
...
3. **Code-only changes deploy themselves.** A systemd timer on ops-host (`watchfire-autoupdate.timer`) polls ghcr, and when the tag named by `watchfire_image` resolves to a new digest it runs `docker compose pull && up -d` and health-checks the result. Merge to `main` → live within one poll interval.
...
**Pinning disables auto-update, for free.** Set `watchfire_image` to an immutable `sha-<git-sha>` tag and the timer's pull becomes a permanent no-op — the tag never moves. No separate "disable" switch to remember.
...
```bash
cd your-iac-repo/solutions/watchfire
...
```bash
systemctl status watchfire-autoupdate.timer     # is it armed, when does it next fire
journalctl -u watchfire-autoupdate -n 50        # what did the last runs do
systemctl start watchfire-autoupdate.service    # force a check right now
systemctl disable --now watchfire-autoupdate.timer   # stop auto-updating
```

**Rollback**: set `watchfire_image` in `group_vars/all.yml` to an older `sha-<git-sha>` and re-run `./deploy.sh ansible`. Under a minute — and because that tag is immutable, it also pins the deployment until you move it back to `:latest`.

## Secret management (ansible-vault)

Secrets live in **`solutions/watchfire/ansible/group_vars/secrets.yml`**, encrypted with `ansible-vault`. Matches every other your-iac-repo solution; no per-solution tooling drift.

**Files:**

```
solutions/watchfire/ansible/group_vars/
...
openobserve_user: "watchfire@example.com"
...
```bash
cd your-iac-repo/solutions/watchfire/ansible
...
2. Renders `templates/watchfire.env.j2` → `/etc/watchfire/secrets/watchfire.env` on ops-host (mode 600, root-owned).
3. docker-compose references `/etc/watchfire/secrets/watchfire.env` via `env_file`.
4. Container reads env at startup. Secrets stay in process memory — no disk echo beyond the env file itself.

The `/etc/watchfire/secrets/` path is covered by the safety hook's secret-read hard-block (spec 08), so the agent can't `cat` its own env.
...
- `watchfire-bootstrap` SP + `watchfire-github-oidc` SP — dropped.
- `watchfire-reader-main` / `watchfire-reader-initech` SPs — deferred until the Azure adapter lands; then they become entries in `all.yml` + `secrets.yml` following the established pattern.
- `/etc/watchfire/bootstrap.env` — dropped. Container reads its env directly.

## Alerting (dead-man switch)

Watchfire emits an OpenTelemetry heartbeat metric at the end of each successful nightly (`watchfire.nightly.completed`, tagged `status=success|truncated`, `run_id`). SigNoz — which already runs on ops-host — is configured with one alert: if no heartbeat is seen in 25 hours, it emails the operator.
...
4. In NPM UI on ops-host: add a proxy host `watchfire.example.com` → `watchfire:8080` (HTTPS, "Force SSL", "Cache Assets").
5. Verify: `https://watchfire.example.com/health` returns `{"ok":true}`; first 🌙 lands in Telegram after the catch-up nightly fires.
